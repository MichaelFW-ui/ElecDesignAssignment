...
void Steer_OnCommandLine(Remote_DataStructure *data) {
    Steer_GetPosition();
    SteerDestinationPos = data->Direction / 5;
    if (SteerDestinationPos > -1000 && SteerDestinationPos < 1000)
        SteerDestinationPos = 0;
...
#define STEER_PID_INTEGRAL_OVERFLOW 60000
...
void Steer_PID_Operate() {
    ...
    Output = Steer_PID.Kp * Error +
             Steer_PID.Kd * (Error - Steer_PID.LastError) +
...
if (!(cnt = (cnt + 1) % 200)) printf("Steer %d %d\r\n", SteerAccelDirection, Output);
